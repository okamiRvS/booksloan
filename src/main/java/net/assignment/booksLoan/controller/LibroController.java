...
public class LibroController {
	...
	public String eliminaId(@PathVariable(name = "id") int id) {
		try {
			...
			if (libro.getListaCopie().size() != 0) {
				for (Copia copia : libro.getListaCopie()) {
					if (!copia.getDisponibilita()) {
	...
	@RequestMapping("/elimina_sequel/{id}/{id_2}")
	public String eliminaSequel(@PathVariable(name = "id") int id, @PathVariable(name = "id_2") int id_2) {
		libroService.deleteSequel(id, id_2);
		return "redirect:/";
	}
